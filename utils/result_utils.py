...
def test_keypoint(data_test, device, model, output_pred=False, output_temporal=False):
    ...
    MPJPE_all_array = np.concatenate(MPJPE_all_list, axis=0)
    PAMPJPE_all_array = np.concatenate(PAMPJPE_all_list, axis=0)
    PCC_all_array = np.concatenate(PCC_all_list, axis=0)
    PCK_all_array = np.concatenate(PCK_all_list, axis=0)
    MAE_all_array = np.concatenate(MAE_all_list, axis=0)
    PCK25_all_array = np.concatenate(PCK25_all_list, axis=0)
    PCK35_all_array = np.concatenate(PCK35_all_list, axis=0)
    ...
    if output_pred:
        y_test_array = np.concatenate(y_test, axis=0)
        y_test_pred_array = np.concatenate(y_test_pred, axis=0)
